...
//		return std::distance(begin,end)*traits::minDistance;
	...
	template<class T>
	void scale(T begin,T end,bounds_t bounds,/*float*/std::pair<distance,distance> ratio)
	{
		if(begin!=end)
		{
...
//				(*begin)=bounds.bind(low+position(((*begin)-low)*ratio));
				(*begin)=bounds.bind(low+position(::MulDiv((*begin)-low,ratio.first,ratio.second)));
...
//			float ratio=float(bounds.distance())/prevDist;
			std::pair<distance,distance> ratio(bounds.distance(),prevDist);
...
//			float ratio=float(bounds.distance())/(prevDist+diff);
			std::pair<distance,distance> ratio(bounds.distance(),prevDist+diff);
			scale(m_separators.begin(),m_separators.end(),bounds,ratio);
		}
	}
	template<class P>
