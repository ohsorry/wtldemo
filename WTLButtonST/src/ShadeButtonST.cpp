...
DWORD CShadeButtonST::OnDrawBackground(CDCHandle pDC, LPCRECT pRect)
{
	if (m_bMouseOnButton)
	{
		if (m_bIsPressed)
			m_dOver.Draw(pDC,1,1);
		else
			m_dOver.Draw(pDC,0,0);
	}
	else
		m_dNormal.Draw(pDC,0,0);

	return BTNST_OK;
} // End of OnDrawBackground

DWORD CShadeButtonST::OnDrawBorder(CDCHandle pDC, LPCRECT pRect)
{
